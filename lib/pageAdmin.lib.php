...
class PageAdminLib{
	...
	public $moduleSymbol = "admin.php";
	
	public $ctrl = "";
	public $ac = "";
	
	function __construct($mTotalData, $mShowPageNum = 20 ){
...
// 			$this->mShowHTML = $this->getEverypageModule();
	...
	function getFirstPageModule(){
		if(!$this->mCurrPage || 1 == $this->mCurrPage){
			$imgUrl = '<div class="body_pagebox">首页</div>';
		}else{
			$imgUrl = '<div class="body_pagebox">首页</div>';
		}
	...
	function getLastPageModule(){
		if($this->mCurrPage == $this->mTotalPage || 1 == $this->mTotalPage){
			$imgUrl = '<div class="body_pagebox">尾页</div>';
		}else{
			$imgUrl ='<div class="body_pagebox">尾页</div>';
		}
	...
	function getPrevPageModule(){
		if($this->mCurrPage > 1){
			...
			$imgUrl = '<div class="body_pagebox">上一页</div>';
		}else{
			$page = 1;
			$imgUrl = '<div class="body_pagebox">上一页</div>';
		}
	...
	function getNextPageModule(){
		if($this->mCurrPage < $this->mTotalPage){
			$page = $this->mCurrPage + 1;
			$imgUrl =  '<div class="body_pagebox">下一页</div>';
		}else{
	...
	function getMiddlePageModule(){
		...
		for($i=$firstLoca;$i<=$lastLoca;$i++){
			if($i == $this->mCurrPage){
				$html .= " ".$this->getHtmlLink($i,'<div class="body_pagebox">'.$i. '</div>');
			}else{
				$html .= " ".$this->getHtmlLink($i,'<div class="body_pagebox">'.$i. '</div>');
			}
		}
